...
describe('Ao tentar salvar uma transferência inválida...', () => {
  ...
  test('Não deve inserir se as contas de origem e detino forem as mesmas', () => template({ acc_dest_id: 10000 }, 'Não é possível transferir de uma conta para ela mesma'));
...
describe('Ao remover transferência', () => {
  ...
  test('O registro deve ser apagado do banco', () => {
    return app.db('transfers').select().where({ id: 10000 })
      .then((result) => {
  ...
  test('As transações associadas devem ter sido removidas', () => {
    return app.db('transactions').select().where({ transfer_id: 10000 })
      .then((result) => {
        expect(result).toHaveLength(0);
      });
  });
